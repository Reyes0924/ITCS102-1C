...
while True:
    ...
    if elite == 'a':
        ...
        for id, info in crush_record.items():
        ...
        for each_student in crush_record.key():
            if search_id in crush_record.key():
                print(f"\n\nRECORD FOUND for {search_id}")
                print("----------------------")
                for id in crush_record[search_id]:
                    print(f" --{id} ")
        ...
        search_id = input("INPUT RECOND ID: ").lower()
        
        for each_student in crush_record.keys():
            if search_id in crush_record.keys():
                ...
                print("----------------------")
                crush_record.pop(search_id)
                print("RECORD DELETED")
            else:
                print("NO RECOND FOUND")
            break
        continue
        ...
        for each_student in crush_record.keys():
            if search_id in crush_record.keys():
                ...
                crush_record[search_id][0] = first_name
                crush_record[search_id][0] = last_name
                crush_record[search_id][0] = course
                crush_record[search_id][0] = section
                crush_record[search_id][0] = gmail
